...
export const make = Effect.fn("cloud.server_self_update.make")(function* () {
  ...
  const update: ServerSelfUpdate["Service"]["update"] = Effect.fn(
    ...
    if (capability === null) {
      return yield* failWith(
        `Remote updates require the ${ALPHA_DISTRIBUTION.productName} background service. Run \`${ALPHA_DISTRIBUTION.serverBinaryName} service install\` on the server machine.`,
      );
    }
