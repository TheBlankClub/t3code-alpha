...
/**
 * A pinned runtime is an exact Alpha server package npm-installed into
 ...
 * switching over, never `npx t3code-alpha`, whose cache is ephemeral and whose
...
export function pinnedRuntimePaths(
  ...
  return {
    ...
    entryPath: path.join(
      versionDir,
      "node_modules",
      ALPHA_DISTRIBUTION.serverPackageName,
      "dist",
      "bin.mjs",
    ),
...
/**
 * Installs an exact Alpha server version into the pinned runtime directory unless a complete
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  return yield* Effect.gen(function* () {
    const installStep = "installing the pinned Alpha runtime (this can take a few minutes)";
    yield* runner
      .run({
        command: "npm",
        args: [
          ...
          "--no-fund",
          "--no-audit",
          `${ALPHA_DISTRIBUTION.serverPackageName}@${input.version}`,
        ],
