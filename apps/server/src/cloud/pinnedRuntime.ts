...
/**
 * A pinned runtime is an exact Alpha release archive unpacked into
 ...
 * Node or npm on the machine; the only npm involvement in T3 Code Alpha is the `t3code-alpha`
 * package for people who prefer `npx t3code-alpha` or `npm install -g t3code-alpha`, and even a
...
export function pinnedRuntimePaths(
  ...
  return {
    ...
    entryPath: path.join(
      versionDir,
      platform === "win32"
        ? `${ALPHA_DISTRIBUTION.serverBinaryName}.exe`
        : ALPHA_DISTRIBUTION.serverBinaryName,
    ),
...
/**
 * Installs the Alpha release archive for `version` into the pinned runtime
...
const installFromArchive = Effect.fn("cloud.pinned_runtime.install_archive")(function* (
  ...
  if (platformKey === undefined) {
    return yield* new PinnedRuntimeInstallError({
      step: `selecting an Alpha release archive for ${input.platform}-${input.arch}`,
    });
  }
  ...
  const checksums = parseChecksums(
    new TextDecoder().decode(
      yield* fetchReleaseAsset(
        ...
        "downloading the Alpha release checksums",
      ),
    ),
  );
  ...
  if (expected === undefined) {
    return yield* new PinnedRuntimeInstallError({
      step: `finding ${fileName} in the Alpha release checksums`,
    });
  }
  const archive = yield* fetchReleaseAsset(
    httpClient,
    `${baseUrl}/${fileName}`,
    "downloading the Alpha release archive",
    input.onProgress,
  );
  ...
  const digest = yield* Effect.tryPromise({
    try: () => crypto.subtle.digest("SHA-256", archive),
      ...
      new PinnedRuntimeInstallError({ step: "verifying the Alpha release archive", cause }),
  });
  if (Encoding.encodeHex(new Uint8Array(digest)) !== expected) {
    return yield* new PinnedRuntimeInstallError({
      step: "verifying the Alpha release archive checksum",
    });
  }
  ...
  yield* fs
    ...
    .pipe(
      Effect.mapError(
        (cause) =>
          new PinnedRuntimeInstallError({ step: "writing the Alpha release archive", cause }),
      ),
    );
  input.onProgress?.({ stage: "extract" });
  const extractStep = "extracting the Alpha release archive";
  // The archive wraps everything in one directory named after its stem;
  // strip it so the executable lands at its runtime root.
  yield* input.runner
    .run({
