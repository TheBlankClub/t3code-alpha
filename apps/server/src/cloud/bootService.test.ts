...
const linuxPlan = {
  ...
  unitPath: "/home/theo/.config/systemd/user/t3code-alpha.service",
};
...
const macPlan = {
  ...
  unitPath: "/Users/theo/Library/LaunchAgents/com.theblankclub.t3code.alpha.service.plist",
};
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable t3code-alpha.service")
        control.enabled = true;
      if (!failed && command === "systemctl --user restart t3code-alpha.service")
        control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop t3code-alpha.service" ||
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop t3code-alpha.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now t3code-alpha.service")).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? [
                "systemctl --user stop t3code-alpha.service",
                "systemctl --user restart t3code-alpha.service",
              ]
            : [
                "launchctl bootout --wait gui/501/com.theblankclub.t3code.alpha.service",
  ...
  it.effect("install with start=false rewrites the files and marks a restart pending", () =>
    Effect.gen(function* () {
      ...
      // the reason is named so `t3-alpha service status` can point at restart.
  ...
  it.effect("restart stops and starts an installed service, and is a no-op otherwise", () =>
    Effect.gen(function* () {
      ...
      expect(
        ...
        "systemctl --user enable t3code-alpha.service",
        "systemctl --user restart t3code-alpha.service",
      ]);
    }),
  );
    ...
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(
          path.join("Library", "LaunchAgents", "com.theblankclub.t3code.alpha.service.plist"),
        ),
      ).toBe(true);
      expect(yield* fs.readFileString(plan.unitPath)).toContain(
      ...
      expect(
        timeouts.get("launchctl bootout --wait gui/501/com.theblankclub.t3code.alpha.service"),
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/com.theblankclub.t3code.alpha.service",
        "launchctl enable gui/501/com.theblankclub.t3code.alpha.service",
