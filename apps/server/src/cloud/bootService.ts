...
// (`com.theblankclub.t3code.alpha`), so launchd and TCC records never collide.
const BOOT_SERVICE_LAUNCHD_LABEL = `${ALPHA_DISTRIBUTION.desktopAppId}.service`;
...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    ...
    case "service-disabled":
      return `The service is not enabled to start automatically. Run \`${ALPHA_DISTRIBUTION.serverBinaryName} service update\` to repair it.`;
    case "service-stopped":
      return `The service is not running. Check the service log and \`systemctl --user status ${BOOT_SERVICE_UNIT_FILE}\`, then run \`${ALPHA_DISTRIBUTION.serverBinaryName} service update\`.`;
  }
}
