...
const BOOT_SERVICE_UNIT_FILE = ALPHA_DISTRIBUTION.serverSystemdUnitName;
// `.service` suffix keeps the label distinct from the desktop app's bundle id
// (com.t3tools.t3code), so launchd and TCC records never collide.
const BOOT_SERVICE_LAUNCHD_LABEL = ALPHA_DISTRIBUTION.serverLaunchdServiceLabel;
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    "[Unit]",
    `Description=${ALPHA_DISTRIBUTION.productName} server`,
...
export class BootServiceInstallError extends Schema.TaggedError<BootServiceInstallError>()(
  ...
  override get message(): string {
    return `Could not set up the ${ALPHA_DISTRIBUTION.productName} background service.`;
  }
}
...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    ...
    case "linger-disabled":
      return `Lingering is disabled. ${ALPHA_DISTRIBUTION.productName} will stop when your last login session ends and will not start at boot. Run \`sudo loginctl enable-linger "$(id -un)"\` on this machine, then retry the service command as your normal user.`;
    case "service-disabled":
      return `The service is not enabled to start automatically. Run \`${ALPHA_DISTRIBUTION.serverBinaryName} service install\` to repair it.`;
    case "service-stopped":
      return `The service is not running. Check the service log and \`systemctl --user status ${BOOT_SERVICE_UNIT_FILE}\`, then run \`${ALPHA_DISTRIBUTION.serverBinaryName} service install\`.`;
    case "restart-pending":
      return `A newer version is installed but the service is still running the previous one. Run \`${ALPHA_DISTRIBUTION.serverBinaryName} service restart\` to switch.`;
  }
}
...
export class BootServiceDowngradeRefusedError extends Schema.TaggedError<BootServiceDowngradeRefusedError>()(
  ...
  override get message(): string {
    return `Refusing to replace ${ALPHA_DISTRIBUTION.serverPackageName}@${this.installedVersion} with older ${ALPHA_DISTRIBUTION.serverPackageName}@${this.targetVersion}. Run the command again with --allow-downgrade to continue.`;
  }
}
            ...
            Effect.mapError(
              (cause) =>
                new PinnedRuntimeInstallError({
                  step: "verifying the pinned Alpha runtime",
                  cause,
                }),
            ),
