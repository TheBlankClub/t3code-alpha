/**
 * `t3-alpha pair` - mint a pairing token for an already-running server and print it
...
const probeEnvironmentDescriptor = (
  ...
  Effect.gen(function* () {
    ...
    // it as unreachable lets `t3-alpha pair --tailscale` repair its own mapping
...
const discoverPairTarget = Effect.fn("pair.discoverPairTarget")(function* (
  ...
  if (explicitBaseDir !== undefined && explicitBaseDir.trim().length > 0) {
    ...
    // outranks the shared home, so `t3-alpha pair` in a worktree pairs with the dev
