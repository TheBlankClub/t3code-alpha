...
const runUpdate = Effect.fn("cli.update.run")(function* (input: {
  ...
  if (targetChannel === "preview" && currentChannel !== "preview") {
    yield* Console.log(
      [
        `${ALPHA_DISTRIBUTION.serverPackageName}@${targetVersion} is a preview build.`,
        ...
        `  will have to switch back to ${currentChannel} yourself with \`${ALPHA_DISTRIBUTION.serverBinaryName} update --channel ${currentChannel} --allow-downgrade\`.`,
      ].join("\n"),
    );
  ...
  if (!input.allowDowngrade && compareExactServiceVersions(targetVersion, newestInstalled) < 0) {
    return yield* new CliUpdateError({
      reason: `${ALPHA_DISTRIBUTION.serverPackageName}@${targetVersion} is older than the installed ${newestInstalled}. Pass --allow-downgrade to install it anyway.`,
    });
  }
  ...
  progress.heading(
    executableCurrent && restartPending
      ...
      : executableCurrent
        ...
        : alreadyOnDisk
          ? "Switching T3 Code Alpha"
          : "Updating T3 Code Alpha",
    executableCurrent
      ? ""
  ...
  if (serviceInstalled && !serviceCurrent) {
    ...
    if (input.assumeYes) {
      ...
      yield* Console.log(
        "  Not a terminal, so the service keeps running its current version. Rerun with --yes to restart it now, or run `t3-alpha service restart` later.",
      );
    }
  }
  ...
  const runtime = yield* ensurePinnedRuntimeInstalled({
    ...
    Effect.catchIf(
      ...
      () =>
        Effect.fail(
          new CliUpdateError({
            reason: `No release archive was published for ${ALPHA_DISTRIBUTION.serverPackageName}@${targetVersion}.`,
          }),
        ),
    ),
  );
  ...
  if (serviceInstalled && !serviceCurrent) {
    ...
    yield* BootService.BootService.pipe(
      ...
      Effect.mapError(
        (error) =>
          new CliUpdateError({
            reason: `${ALPHA_DISTRIBUTION.serverPackageName}@${targetVersion} is installed but the background service could not be ${restartService ? "updated" : "pointed at it"}: ${error.message}`,
          }),
      ),
    );
  ...
  progress.success(`Installed T3 Code Alpha ${targetVersion}`);
  if (Option.isSome(repointed)) {
    yield* Console.log(`  Run ${ALPHA_DISTRIBUTION.serverBinaryName} to get started.\n`);
  } else {
  ...
  if (serviceUpdated) {
    ...
    yield* Console.log(
      `  Background service still running ${serviceVersion ?? "an unknown version"}. Run \`${ALPHA_DISTRIBUTION.serverBinaryName} service restart\` when you are ready to switch it to ${targetVersion}.`,
    );
