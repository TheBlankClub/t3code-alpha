...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
  ...
  it.effect("ignores persisted remote observability settings in Alpha", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
  ...
  it.effect(
    "keeps Alpha export disabled when T3CODE_OTEL_SDK_DISABLED overrides the ambient switch",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
  ...
  it.effect(
    "ignores T3CODE and standard OTEL endpoints while Alpha outbound telemetry is disabled",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
  ...
  it.effect("keeps export disabled when an OTEL endpoint is unreadable and a T3 URL is set", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({
