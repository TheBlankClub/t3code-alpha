...
const resolveClientFilePath = Effect.fn("AntigravityAdapter.resolveClientFilePath")(
  function* (input: {
    ...
    // Follow symlinks through the nearest existing parent. A write may create
    // several directories, so its immediate parent can be absent even when an
    // existing ancestor is a symlink outside the workspace.
    ...
    while (
      !(yield* input.fileSystem.exists(existingParent).pipe(Effect.mapError(outsideWorkspaceError)))
    ) {
    ...
    const realParent = yield* input.fileSystem
      .realPath(existingParent)
      .pipe(Effect.mapError(outsideWorkspaceError));
    const real = path.join(realParent, ...missingSegments, path.basename(resolved));
