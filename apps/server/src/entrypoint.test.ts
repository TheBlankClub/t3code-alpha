...
describe("isEntrypoint", () => {
  ...
  it("canonicalizes a symlinked parent in the module URL", () => {
    const dir = makeTempDir();
    const realDir = NodePath.join(dir, "real");
    const aliasDir = NodePath.join(dir, "alias");
    NodeFS.mkdirSync(realDir);
    NodeFS.symlinkSync(realDir, aliasDir, "dir");
    const real = NodePath.join(realDir, "bin.mjs");
    const entry = NodePath.join(realDir, "t3");
    NodeFS.writeFileSync(real, "");
    NodeFS.symlinkSync(real, entry);

    expect(
      isEntrypoint({
        moduleUrl: NodeURL.pathToFileURL(NodePath.join(aliasDir, "bin.mjs")).href,
        entryPath: entry,
        runtimeMain: undefined,
      }),
    ).toBe(true);
