...
describe("ServerLoggerLive", () => {
  it.effect("ignores a configured logs endpoint", () =>
    Effect.gen(function* () {
  ...
  it.effect("ignores configured protobuf export and headers", () =>
    Effect.gen(function* () {
      const requests = yield* logThrough({
        otlpLogsUrl: "https://collector.example.com/v1/logs",
        otlpLogsExport: {
  ...
  it.effect("keeps log messages on the local span despite a configured logs endpoint", () =>
    Effect.gen(function* () {
      const { requests, spans } = yield* logInSpanThrough({
        otlpLogsUrl: "https://collector.example.com/v1/logs",
      });
