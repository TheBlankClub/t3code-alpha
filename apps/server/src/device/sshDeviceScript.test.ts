  ...
  it.effect("reuses its own healthy helpers and stops only its own runtime", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(async () => {
        ...
        const root = NodePath.join(home, ".t3-alpha/device");
        const officialTool = NodePath.join(home, ".t3/device/tools/expo-device-hub@0.0.1");
        await NodeFSP.mkdir(
          NodePath.join(officialTool, "node_modules/expo-device-hub/dist/server"),
          {
            recursive: true,
          },
        );
        await NodeFSP.writeFile(NodePath.join(officialTool, ".install-complete"), "0.0.1");
        const officialEntry = NodePath.join(
          officialTool,
          "node_modules/expo-device-hub/dist/server/cli.mjs",
        );
        await NodeFSP.writeFile(officialEntry, "official device tool");
...
`,
        ...
        try {
          ...
          expect(await NodeFSP.readFile(officialEntry, "utf8")).toBe("official device tool");
          expect(await NodeFSP.readdir(NodePath.join(home, ".t3/device"))).toEqual(["tools"]);
        } finally {
