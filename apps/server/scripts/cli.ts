...
const buildExeCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      yield* Effect.log(
        `[cli] Built dist-exe/${ALPHA_DISTRIBUTION.serverBinaryName} (expects client/, resource-monitor/, and the runtime-external node_modules beside it; scripts/build-cli-archive.ts assembles that tree)`,
      );
    }),
).pipe(
