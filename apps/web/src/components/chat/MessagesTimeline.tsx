...
interface MessagesTimelineProps {
  ...
  onScrollRestored?: (restore: { hasNewContent: boolean; atEnd: boolean }) => void;
...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  useLayoutEffect(() => {
    ...
    const restoreReadingPosition = position?.atEnd === false && index >= 0;
    if (restoreReadingPosition) onManualNavigation();
    if (position?.atEnd === false && index < 0) {
      rememberTimelinePosition(listIdentityKey, { ...position, atEnd: true });
    }
    onScrollRestored?.({
      atEnd: !restoreReadingPosition,
      hasNewContent:
        restoreReadingPosition &&
        position.lastRowId !== undefined &&
        rows.at(-1)?.id !== position.lastRowId,
    });
    ...
    if (position && state && isAtEnd !== undefined) {
      ...
      if (row && element) {
        rememberTimelinePosition(listIdentityKey, {
          ...
          // Streaming can briefly grow past the viewport while the user still follows the end.
          atEnd: isAtEnd || liveFollowEnabled,
          lastRowId: rows.at(-1)?.id,
          disclosures: {
