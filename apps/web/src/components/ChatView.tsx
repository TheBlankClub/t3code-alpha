...
export default function ChatView(props: ChatViewProps) {
  ...
  const onTimelineScrollRestored = useCallback(
    (restore: { hasNewContent: boolean; atEnd: boolean }) => {
      isAtEndRef.current = restore.atEnd;
      timelineScrollModeRef.current = restore.atEnd ? "following-end" : "free-scrolling";
      liveFollowUserScrollGenerationRef.current = restore.atEnd
        ? anchorUserScrollGenerationRef.current
        : null;
      setTimelineLiveFollowEnabled(restore.atEnd);
      showScrollDebouncer.current.cancel();
      setShowScrollToBottom(!restore.atEnd);
      setScrollToEndHasNewContent(restore.hasNewContent);
    },
    [],
  );
