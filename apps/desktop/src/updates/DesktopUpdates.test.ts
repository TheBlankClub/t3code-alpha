...
describe("DesktopUpdates", () => {
  ...
  it.effect("updates Linux .deb installs and leaves other non-AppImage installs off", () =>
    Effect.gen(function* () {
      const linuxState = (packageType: string | undefined, appVersion?: string) =>
        Effect.scoped(
          Effect.gen(function* () {
            const updates = yield* DesktopUpdates.DesktopUpdates;
            yield* updates.configure;
          ...
          Effect.provide(
            makeHarness({ platform: "linux", packageType, ...(appVersion ? { appVersion } : {}) })
              .layer,
          ),
        );
      ...
      const alphaDeb = yield* linuxState("deb\n", "1.2.4-alpha.20260925.1");
      assert.equal(alphaDeb.channel, "alpha");
      assert.equal(alphaDeb.status, "disabled");
