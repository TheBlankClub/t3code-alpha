...
describe("DesktopObservability", () => {
  ...
  it.effect("keeps main process log records local despite configured endpoints", () => {
    ...
    return Effect.gen(function* () {
      ...
      const environmentLayer = makeEnvironmentLayer(baseDir, true, {
        ...
        T3CODE_OTLP_TRACES_URL: "https://collector.example.com/v1/traces",
      ...
      assert.notEqual(record, undefined);
      assert.lengthOf(record?.events ?? [], 1);
    }).pipe(
      Effect.scoped,
  ...
  it.effect("ignores a standard OTEL endpoint while Alpha outbound telemetry is disabled", () => {
    const requests: Array<ExportedRequest> = [];
    return Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
  ...
  it.effect("keeps kill switch warnings local despite a configured endpoint", () => {
    const requests: Array<ExportedRequest> = [];
    return Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
  ...
  it.effect("ignores every signal endpoint persisted in Settings", () => {
    const requests: Array<ExportedRequest> = [];
    return Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
