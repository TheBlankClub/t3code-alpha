...
const makeDesktopClerkLayer = (isDevelopment = true, events: string[] = []) => {
  const environment = DesktopEnvironment.DesktopEnvironment.of({
    ...
    userDataDirName: isDevelopment ? "t3code-dev" : "t3code-alpha",
    legacyUserDataDirName: isDevelopment ? "T3 Code (Dev)" : "t3code-alpha",
...
describe("DesktopClerk", () => {
  ...
  it.effect("configures the packaged SDK with the Alpha renderer origin", () => {
    storageMock.mockReturnValue(storageAdapter);
    createClerkBridgeMock.mockReturnValue({ cleanup: vi.fn(), isPrimaryInstance: true });
        ...
        [
          {
            storage: storageAdapter,
            passkeys: true,
            renderer: { scheme: "t3code-alpha", host: "app" },
          },
        ],
      ]);
    });
