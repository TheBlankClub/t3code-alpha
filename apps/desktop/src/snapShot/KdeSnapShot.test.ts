...
it("installs offline at a stable path and verifies permissions using the installed identity", async () => {
  ...
  expect(executable).toContain("t3code-alpha/kde-capture");
  expect(desktop).toContain("com.theblankclub.t3code.alpha.KdeCapture.desktop");
  ...
  expect(kdeCaptureDesktopEntry(executable)).toContain("Name=T3 Code Alpha SnapShots");
