...
export const buildWslRuntimeInstallScript = (
  ...
  return [
    "set -eu",
    `runtime_parent="$HOME/${ALPHA_DISTRIBUTION.serverHomeDirName}/wsl-runtime"`,
    ...
    "runtime_entry_runs() {",
    `  [ -x "$1/${ALPHA_DISTRIBUTION.serverBinaryName}" ] && "$1/${ALPHA_DISTRIBUTION.serverBinaryName}" --version >/dev/null 2>&1`,
    "}",
    ...
    `  sha256sum "$1/${ALPHA_DISTRIBUTION.serverBinaryName}" 2>/dev/null | cut -d ' ' -f 1`,
    "}",
    "runtime_is_ready() {",
    '  [ -f "$ready_marker" ] &&',
...
export const buildWslRuntimeInvalidateScript = (runtimeId: string): string => {
  ...
  return [
    "set -eu",
    `rm -f "$HOME/${ALPHA_DISTRIBUTION.serverHomeDirName}/wsl-runtime/${safeRuntimeId}/${WSL_RUNTIME_READY_MARKER}"`,
  ].join("\n");
};
...
export const buildWslRuntimeProbeScript = (linuxAppRoot: string) =>
  [
    ...
    `${shellQuote(`${linuxAppRoot}/${ALPHA_DISTRIBUTION.serverBinaryName}`)} --version >/dev/null 2>&1`,
  ].join("\n");
...
const probeWslRuntimeImpl = (
  ...
  Effect.gen(function* () {
    ...
    if (probe.exitCode !== 0) {
      ...
      return {
        ok: false,
        reason: `${linuxAppRoot}/${ALPHA_DISTRIBUTION.serverBinaryName} --version failed (exit ${probe.exitCode})${trimmedTail ? `: ${trimmedTail}` : ""}`,
      } as const;
    }
