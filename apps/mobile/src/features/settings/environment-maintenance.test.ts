...
describe("environment release checks", () => {
  ...
  it("keeps Alpha hosts on the fork's Alpha releases", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        Response.json([
          { tag_name: "v2.0.0" },
          { tag_name: "v1.0.0-nightly.20260924.9" },
          { tag_name: "v1.0.0-alpha.20260924.2" },
        ]),
      );
    vi.stubGlobal("fetch", fetchMock);
    expect(await findEnvironmentUpdate("1.0.0-alpha.20260924.1", signal)).toBe(
      "1.0.0-alpha.20260924.2",
    );
    expect(fetchMock.mock.calls[0]?.[0]).toBe(
      "https://api.github.com/repos/TheBlankClub/t3code-alpha/releases?per_page=100&page=1",
    );
  });
