...
- 2026-09-26, upstream `ed809f7ad274ebccdfbf029ccfd7ff2582fd486a`: `mechanical-conflict`. Adopted closing idle shells when a thread settles. Fresh-shell initial commands, acknowledgement, restart handling, and the older-server write fallback remain.

- 2026-09-25, upstream `e5a46d6c5d00b89afba5274a94d42428c8d79763`: `unaffected`. The new shell-command action uses the existing project script path. Initial-command acknowledgement and fallback behavior remain intact.
...
- 2026-09-20, upstream `7445aa733ada33e45289e5aa5055f79142556513`: `unaffected`. The ChatView question-answer change preserves initial-command handling and the older-server fallback.
...
- 2026-09-12, upstream `efccda9ac9230db22b36990cffabdad218fa41b0`: `mechanical-overlap`.
  Adopted scoped project-action settings and terminal manager changes while retaining the fresh-shell
  initial-command acknowledgement, restart handling, and older-server fallback.
...
- 2026-09-26, upstream `95030dc674883f0f2a7fd034b32ce742c8cf55d0`: `unaffected`. The `node-pty` prebuild bump does not change the initial-command handshake or the older-server fallback.
