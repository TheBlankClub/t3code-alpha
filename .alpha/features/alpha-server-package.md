...
tests:
  ...
  - vp test run packages/shared/src/npmCliInstaller.test.ts scripts/install.test.ts packages/shared/src/cliRelease.test.ts
---
...
- npm installs one `t3code-alpha` package for macOS arm64 and Linux arm64/x64. Its installer
  downloads the matching GitHub release archive and verifies it against `SHA256SUMS`.
  Legacy npm service entrypoints dispatch to the installed executable.
...
- The source workspace keeps upstream's internal `t3` package name for Effect service keys.
  Packaging emits one Alpha npm manifest with license metadata. The publisher uploads one
  tarball and preserves interactive npm authentication.
...
- 2026-09-24, upstream `f26ee083fe41720207fb10616359500dcecc19d3`: `unaffected`. Authentication and settings appearance changes preserve Alpha package and update identities.
...
- 2026-09-20, upstream `7445aa733ada33e45289e5aa5055f79142556513`: `unaffected`. Provider startup diagnostics and Claude continuation keys preserve Alpha package, binary, and service identities.
...
- 2026-09-13, upstream `20363c32c9bfdbf49c2716ef11d1f18483fcc01b`: `persistence-sensitive`.
  Adopted upstream message-context contracts and projections while retaining Alpha package and server identities.

- 2026-09-12, upstream `e816064945144957b6eb9b268912a98b0555644b`: `unaffected`. No incoming
  change touches the Alpha server package, binary, or remote package selection.

- 2026-09-12, upstream `efccda9ac9230db22b36990cffabdad218fa41b0`: `mechanical-conflict`.
  Adopted the pinned Effect package fix and new server runtime while retaining `t3-alpha`, the
  fork repository metadata, Alpha build channel, and remote package selection.
...
- 2026-09-10, upstream `d29c56a5c404cb0f58d3b2ac41762fa0d0ac28d4`: `mechanical-conflict`.
  ...
  `t3code-alpha` package, `t3-alpha` binary, Alpha home, remote install, and service identities.
