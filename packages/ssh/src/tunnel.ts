...
require_installed_t3_cli() {
  if ! T3_CLI_PATH="$("$@" -- sh -c 'command -v @@T3_BINARY_NAME@@')"; then
  ...
  printf 'Remote host installed %s but npm produced no @@T3_BINARY_NAME@@ executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@T3_PACKAGE_SPEC@@ >&2
  return 1
}
