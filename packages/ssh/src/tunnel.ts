...
T3_RUNTIME_DIR="$HOME/@@T3_SERVER_HOME_DIR@@/runtime/versions/$T3_ARCHIVE_VERSION"
t3_runtime_ready() {
  [ -x "$T3_RUNTIME_DIR/@@T3_SERVER_BINARY_NAME@@" ] && [ "$(cat "$T3_RUNTIME_DIR/.install-complete" 2>/dev/null)" = "$T3_ARCHIVE_VERSION" ]
}
if ! t3_runtime_ready; then
  mkdir -p "$HOME/@@T3_SERVER_HOME_DIR@@/runtime/versions"
  ...
  # under it.
  T3_LOCK="$HOME/@@T3_SERVER_HOME_DIR@@/runtime/versions/.$T3_ARCHIVE_VERSION.install.lock"
...
if ! t3_runtime_ready; then
  ...
  T3_ARCHIVE="@@T3_RELEASE_ARTIFACT_PREFIX@@-$T3_ARCHIVE_VERSION-$T3_PLATFORM-$T3_ARCH.tar.gz"
  T3_STAGING="$(mktemp -d "$HOME/@@T3_SERVER_HOME_DIR@@/runtime/versions/.staging-XXXXXX")"
  ...
  if ! "$T3_STAGING/@@T3_SERVER_BINARY_NAME@@" --version >/dev/null 2>&1; then
...
exec "$T3_RUNTIME_DIR/@@T3_SERVER_BINARY_NAME@@" "$@"
`;

const REMOTE_LAUNCH_SCRIPT = `set -eu
@@T3_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/@@T3_SERVER_HOME_DIR@@/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/@@T3_SERVER_HOME_DIR@@"
...
STATE_DIR="$HOME/@@T3_SERVER_HOME_DIR@@/ssh-launch/@@T3_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/@@T3_SERVER_HOME_DIR@@"
RUNNER_FILE="$STATE_DIR/run-t3.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteT3RunnerScript(input?: RemoteT3RunnerOptions): string {
  ...
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      ...
      T3_SERVER_HOME_DIR: ALPHA_DISTRIBUTION.serverHomeDirName,
      T3_SERVER_BINARY_NAME: ALPHA_DISTRIBUTION.serverBinaryName,
      T3_RELEASE_ARTIFACT_PREFIX: ALPHA_DISTRIBUTION.serverReleaseArtifactPrefix,
    }),
  );
}
