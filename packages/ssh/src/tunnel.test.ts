...
describe("ssh tunnel scripts", () => {
  it("installs and runs the release archive without Node, npm, or npx", () => {
    ...
    assert.include(
      script,
      "T3_RELEASE_BASE_URL='https://github.com/TheBlankClub/t3code-alpha/releases/download'",
    );
    assert.include(script, 'T3_RUNTIME_DIR="$HOME/.t3-alpha/runtime/versions/$T3_ARCHIVE_VERSION"');
    assert.include(
      script,
      'T3_ARCHIVE="t3-alpha-$T3_ARCHIVE_VERSION-$T3_PLATFORM-$T3_ARCH.tar.gz"',
    );
    assert.include(script, "SHA256SUMS");
    assert.include(script, 'exec "$T3_RUNTIME_DIR/t3-alpha" "$@"');
    ...
    assert.include(
      script,
      'T3_LOCK="$HOME/.t3-alpha/runtime/versions/.$T3_ARCHIVE_VERSION.install.lock"',
    );
    ...
    assert.isBelow(
      script.indexOf('"$T3_STAGING/t3-alpha" --version'),
...
describe("archive runner script", () => {
  ...
  const makeMirror = Effect.fn("makeMirror")(function* (root: string) {
    ...
    const stem = `t3-alpha-${archiveVersion}-${platform}-${arch}`;
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const versionsDir = `${home}/.t3-alpha/runtime/versions`;
