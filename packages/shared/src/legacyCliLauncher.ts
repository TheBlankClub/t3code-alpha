/**
 * `dist/bin.mjs` of the Alpha npm package: the entry point boot-service
...
const executableName = process.platform === "win32" ? "t3-alpha.exe" : "t3-alpha";
const executable = join(dirname(require.resolve("t3code-alpha-" + process.platform + "-" + process.arch + "/package.json")), executableName);
...
const fail = (error) => {
  if (!error) return;
  process.stderr.write("t3-alpha: " + error.message + "\\n");
  child.kill("SIGTERM");
  process.exitCode = 1;
};
if (ipc) {
