...
if [ -f "${target_dir}/.install-complete" ] && [ "$(cat "${target_dir}/.install-complete")" = "$version" ]; then
  printf 't3-alpha %s is already installed at %s\n' "$version" "$target_dir"
else
...
printf 'Installed t3-alpha %s\n  %s -> %s\n' "$version" "${bin_dir}/t3-alpha" "${target_dir}/t3-alpha"
case ":${PATH}:" in
  *":${bin_dir}:"*) ;;
  *) printf 'Add %s to your PATH to run `t3-alpha`.\n' "$bin_dir" ;;
esac
