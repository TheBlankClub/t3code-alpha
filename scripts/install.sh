#!/bin/sh
# Installs the T3 Code Alpha CLI from a GitHub Release archive. Needs only sh, tar,
# sha256sum or shasum, and curl or wget; no Node, npm, or compiler.
#
#   curl -fsSL https://raw.githubusercontent.com/TheBlankClub/t3code-alpha/alpha/scripts/install.sh | sh
#
# Environment:
...
#   T3CODE_INSTALL_BIN_DIR   where the `t3-alpha` symlink goes (default: ~/.local/bin)
...
t3_home="${T3CODE_HOME:-$HOME/.t3-alpha}"
...
if [ -z "$version" ]; then
  ...
  case "$channel" in
    alpha) tag_pattern='v\([0-9][^\"]*-alpha\.[0-9]*\.[0-9]*\)' ;;
    stable) tag_pattern='v\([0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*\)' ;;
    nightly | preview) tag_pattern="v\([0-9][^\"]*-${channel}\.[0-9]*\.[0-9]*\)" ;;
    *) fail "T3CODE_CHANNEL must be alpha, stable, nightly, or preview" ;;
  esac
  tmp_index="$(mktemp)"
...
case "$version" in
  *-preview.*)
    printf '%s\n' \
      "t3-alpha ${version} is a preview build." \
      ...
      "  Set T3CODE_CHANNEL=alpha (the default) for an Alpha build." >&2
...
stem="t3-alpha-${version}-${platform}-${arch}"
archive="${stem}.tar.gz"
...
if [ -f "${target_dir}/.install-complete" ] && [ "$(cat "${target_dir}/.install-complete")" = "$version" ]; then
  ...
  printf '  %sInstalling%s T3 Code Alpha %s%s%s\n\n' "$muted" "$reset" "$bold" "$version" "$reset" >&2
  step "Downloading..."
  fetch_status=0
  fetch "${base_url}/v${version}/SHA256SUMS" "${staging}/SHA256SUMS" || fetch_status=$?
  if [ "$fetch_status" -eq 44 ]; then
    fail "t3-alpha ${version} has no release archive for ${platform}-${arch}; install t3code-alpha with npm or choose a release that includes this platform"
  ...
  "${staging}/t3-alpha" --version >/dev/null || fail "the downloaded executable does not run"
...
mkdir -p "$bin_dir"
ln -sfn "${target_dir}/t3-alpha" "${bin_dir}/t3-alpha"
if "$interactive"; then printf '\r\033[2K' >&2; fi
printf '  %sInstalled T3 Code Alpha %s%s\n\n' "$green" "$version" "$reset" >&2
case ":${PATH}:" in
  *":${bin_dir}:"*) printf '  Run %st3-alpha%s to get started.\n\n' "$bold" "$reset" ;;
  *) printf '  Add %s to your PATH, then run %st3-alpha%s.\n\n' "$bin_dir" "$bold" "$reset" ;;
esac
