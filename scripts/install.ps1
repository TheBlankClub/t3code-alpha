# Installs the T3 Code Alpha CLI from a GitHub Release archive on Windows. Needs
# only PowerShell 5.1+; no Node, npm, or compiler.
#
#   irm https://raw.githubusercontent.com/TheBlankClub/t3code-alpha/alpha/scripts/install.ps1 | iex
#
# Environment:
#   T3CODE_CHANNEL           release train to follow: alpha, stable, nightly, or preview
#                            (default: alpha)
#   T3CODE_VERSION           exact version to install (overrides T3CODE_CHANNEL)
#   T3CODE_HOME              T3 Code Alpha home directory (default: ~\.t3-alpha)
#   T3CODE_INSTALL_BIN_DIR   where t3-alpha.exe is linked (default: ~\.local\bin)
...
# same layout `t3-alpha service install` uses, so the service reuses this download.
...
$t3Home = if ($env:T3CODE_HOME) { $env:T3CODE_HOME } else { Join-Path $HOME ".t3-alpha" }
$binDir = if ($env:T3CODE_INSTALL_BIN_DIR) { $env:T3CODE_INSTALL_BIN_DIR } else { Join-Path $HOME ".local\bin" }

function Fail([string] $message) {
  Write-Error "t3-alpha install: $message"
  exit 1
}
...
if ($interactive) {
  ...
  for ($i = 0; $i -lt $mark.Length; $i++) {
    ...
    $label = if ($i -eq 1) { "     ${bold}T3 Code Alpha$reset" } elseif ($i -eq 2) { "     ${muted}CLI installer$reset" } else { "" }
...
$channel = if ($env:T3CODE_CHANNEL) { $env:T3CODE_CHANNEL } else { "alpha" }
$version = $env:T3CODE_VERSION
if (-not $version) {
  # Tags are v<semver>; Alpha versions carry the alpha prerelease identifier.
  # Only tags of the requested train are considered.
  $tagPattern = switch ($channel) {
    "alpha" { '^v\d+\.\d+\.\d+-alpha\.\d+\.\d+$' }
    "stable" { '^v\d+\.\d+\.\d+$' }
    "nightly" { '^v\d+\.\d+\.\d+-nightly\.\d+\.\d+$' }
    "preview" { '^v\d+\.\d+\.\d+-preview\.\d+\.\d+$' }
    default { Fail "T3CODE_CHANNEL must be alpha, stable, nightly, or preview" }
  }
  $releases = Invoke-RestMethod -Uri "https://api.github.com/repos/$repo/releases?per_page=100" -Headers @{ "User-Agent" = "t3-alpha-install" }
...
if ($version -match '-preview\.') {
  Write-Warning "t3-alpha $version is a preview build. Preview builds are cut by maintainers from unreleased branches to exercise the release pipeline. They can be broken, receive no fixes, and are never offered as updates. Set T3CODE_CHANNEL=alpha (the default) for an Alpha build."
  if ($channel -ne "preview" -and -not $env:T3CODE_VERSION) {
    Fail "refusing a preview build that was not explicitly requested"
  }
}

$stem = "t3-alpha-$version-win32-$arch"
$archive = "$stem.zip"
...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  ...
  try {
    ...
    [Console]::Error.WriteLine("  ${muted}Installing$reset T3 Code Alpha $bold$version$reset`n")
    Step "Downloading..."
    try {
      ...
      if ($status -eq 404) {
        Fail "t3-alpha $version has no release archive for win32-$arch; install t3code-alpha with npm or choose a release that includes this platform"
      }
      throw
    }
    ...
    & (Join-Path $staging "t3-alpha.exe") --version | Out-Null
...
Step "Setting up the t3-alpha command..."
New-Item -ItemType Directory -Force -Path $binDir | Out-Null
$shim = Join-Path $binDir "t3-alpha.cmd"
# UTF-8 without a BOM: cmd.exe reads the shim as-is, and ASCII would corrupt
# non-ASCII characters in the user's home path.
[System.IO.File]::WriteAllText($shim, "@echo off`r`n`"$(Join-Path $targetDir 't3-alpha.exe')`" %*", (New-Object System.Text.UTF8Encoding $false))
if ($interactive) { [Console]::Error.Write("`r$esc[2K") }
[Console]::Error.WriteLine("  ${green}Installed T3 Code Alpha $version$reset`n")
if (($env:PATH -split ";") -notcontains $binDir) {
  Write-Host "  Add $binDir to your PATH, then run ${bold}t3-alpha$reset.`n"
} else {
  Write-Host "  Run ${bold}t3-alpha$reset to get started.`n"
}
