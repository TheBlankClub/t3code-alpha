...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  Write-Host "t3-alpha $version is already installed at $targetDir"
} else {
...
Write-Host "Installed t3-alpha $version"
Write-Host "  $shim -> $(Join-Path $targetDir 't3-alpha.exe')"
if (($env:PATH -split ";") -notcontains $binDir) {
  Write-Host "Add $binDir to your PATH to run t3-alpha."
}
