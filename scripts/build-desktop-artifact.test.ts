...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it.effect("ad-hoc signs unsigned Alpha macOS builds", () =>
    Effect.gen(function* () {
      const config = yield* createBuildConfig(
        "mac",
        "dmg",
  ...
  it("ships the Linux CLI release archive as the WSL runtime", () => {
    ...
    const runtimeArchivePath = "/tmp/t3-alpha-1.2.3-linux-x64.tar.gz";
    ...
    assert.equal(wslRuntimeArchiveStem("1.2.3", "x64"), "t3-alpha-1.2.3-linux-x64");
  });

  it("parses Windows bsdtar member listings with CRLF line endings", () => {
    assert.deepStrictEqual(
      parseWslRuntimeArchiveMembers(
        "./t3-alpha-1.2.3-linux-x64/t3-alpha\r\nt3-alpha-1.2.3-linux-x64/client/\r\n",
      ),
      ["t3-alpha-1.2.3-linux-x64/t3-alpha", "t3-alpha-1.2.3-linux-x64/client"],
    );
  });
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const error = yield* stageWslRuntimeArchive({
          sourceArchivePath: path.join(root, "t3-alpha-1.2.3-linux-x64.tar.gz"),
