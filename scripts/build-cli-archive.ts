 ...
 * archive: the `t3-alpha` binary, the web client, the resource monitor, and a
 ...
 * archive stem):
 *
 *   t3-alpha | t3-alpha.exe  the single-executable
...
import {
  cliArchiveFileName as cliReleaseArchiveFileName,
...
export function cliArchiveStem(version: string, platform: BuildPlatform, arch: BuildArch): string {
  return `${ALPHA_DISTRIBUTION.serverReleaseArtifactPrefix}-${version}-${cliArchivePlatformKey(platform, arch)}`;
}
...
export function cliArchiveFileName(version: string, platform: BuildPlatform, arch: BuildArch) {
  ...
  return cliReleaseArchiveFileName(
    version,
    cliArchivePlatformKey(platform, arch) as CliArchivePlatformKey,
  );
}
...
const signMacArchiveContents = Effect.fn("signMacArchiveContents")(function* (input: {
  ...
  const notarizeZip = path.join(path.dirname(input.executablePath), ".notarize-t3-alpha.zip");
  yield* runCommand(
  ...
  yield* Effect.log("[cli-archive] Notarized t3-alpha.");
});
...
/** Signs t3-alpha.exe through the same Azure Trusted Signing setup the installer uses. */
const signWindowsExecutable = Effect.fn("signWindowsExecutable")(function* (
  ...
  yield* runCommand(
    ...
    "Invoke-TrustedSigning t3-alpha.exe",
  );
  yield* Effect.log("[cli-archive] Signed t3-alpha.exe (Azure Trusted Signing).");
});
...
const buildCliArchive = Effect.fn("buildCliArchive")(function* (input: {
  ...
  const executableName = `${ALPHA_DISTRIBUTION.serverBinaryName}${input.platform === "win" ? ".exe" : ""}`;
  // tsdown suffixes cross-built executables with their target (t3-alpha-darwin-x64);
  // a host build is plain t3-alpha. Prefer the exact target when both exist.
  const targetKey = `${input.platform === "mac" ? "darwin" : input.platform}-${input.arch}`;
  const targetExecutable = path.join(
    serverDir,
    "dist-exe",
    `${ALPHA_DISTRIBUTION.serverBinaryName}-${targetKey}${input.platform === "win" ? ".exe" : ""}`,
  );
  ...
  const stageRoot = yield* fs.makeTempDirectoryScoped({ prefix: "t3-alpha-cli-archive-" });
...
const command = Command.make(
  ...
  Command.withDescription("Package the t3-alpha single-executable into a per-platform archive."),
);
