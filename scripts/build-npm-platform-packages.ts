 ...
 * behind `npx t3code-alpha` / `npm i -g t3code-alpha`: one
 * `t3code-alpha-<platformKey>` package per archive holding the archive's
 * contents verbatim, plus the `t3code-alpha` launcher
 ...
 *
 *   t3code-alpha-<platformKey>/      archive contents flattened + package.json
 *   t3code-alpha-<platformKey>.tgz   the same tree as an npm tarball
 *   t3code-alpha/                    launcher: package.json, bin/t3-alpha.js, README.md
 *   t3code-alpha.tgz                 the launcher as an npm tarball
 *
...
export const NPM_LAUNCHER_PACKAGE_NAME = ALPHA_DISTRIBUTION.serverPackageName;
export const NPM_LAUNCHER_BINARY_NAME = ALPHA_DISTRIBUTION.serverBinaryName;
...
export function npmPlatformPackageName(platformKey: CliArchivePlatformKey): string {
  return `${ALPHA_DISTRIBUTION.serverNpmPlatformPackagePrefix}-${platformKey}`;
}
...
export function npmPlatformPackageManifest(
  ...
  return {
    ...
    version,
    description: `T3 Code Alpha CLI executable for ${platformKey}`,
    license: serverPackageJson.license,
    repository: serverPackageJson.repository,
    os: [os],
    cpu: [cpu],
    files: [
      NPM_LAUNCHER_BINARY_NAME,
      `${NPM_LAUNCHER_BINARY_NAME}.exe`,
      "client",
      "resource-monitor",
      "node_modules",
    ],
...
export function npmPlatformPackageReadme(platformKey: CliArchivePlatformKey): string {
  return [
    ...
    "",
    `The T3 Code Alpha CLI executable for ${platformKey}. Do not install this package directly:`,
    ...
    "```",
    "",
    "Source and documentation: https://github.com/TheBlankClub/t3code-alpha",
    "",
  ].join("\n");
}

/** package.json for the Alpha launcher. No engines: its bin script is trivial CJS. */
export function npmLauncherPackageManifest(
  ...
  return {
    ...
    license: serverPackageJson.license,
    repository: serverPackageJson.repository,
    bin: { [NPM_LAUNCHER_BINARY_NAME]: `./bin/${NPM_LAUNCHER_BINARY_NAME}.js` },
    files: ["bin", "dist"],
...
/**
 * The launcher every `npx t3code-alpha` runs. Plain CommonJS with no dependencies so it
...
let packageDir;
try {
  packageDir = dirname(require.resolve("${ALPHA_DISTRIBUTION.serverNpmPlatformPackagePrefix}-" + key + "/package.json"));
} catch {
  process.stderr.write(
    [
      "t3-alpha: no T3 Code Alpha CLI build is available for this platform (" + key + ").",
      "Supported platforms: " + SUPPORTED.join(", ") + ".",
      "If yours is listed, reinstall t3code-alpha so npm fetches its optional dependency.",
      "The desktop app and release archives are at https://github.com/TheBlankClub/t3code-alpha/releases",
      "",
    ].join("\\n"),
  );
  process.exit(1);
...
const executable = join(packageDir, process.platform === "win32" ? "t3-alpha.exe" : "t3-alpha");
const result = spawnSync(executable, process.argv.slice(2), { stdio: "inherit" });
if (result.error) {
  process.stderr.write("t3-alpha: failed to start " + executable + ": " + result.error.message + "\\n");
  process.exit(1);
}
...
const stagePlatformPackage = Effect.fn("stagePlatformPackage")(function* (input: {
  ...
  const executableName = `${NPM_LAUNCHER_BINARY_NAME}${input.key.startsWith("win32") ? ".exe" : ""}`;
  ...
  if (executableName === NPM_LAUNCHER_BINARY_NAME) {
...
/** Writes the launcher package and its tarball. */
const stageLauncherPackage = Effect.fn("stageLauncherPackage")(function* (input: {
  ...
  const launcherScript = path.join(stageDir, `bin/${NPM_LAUNCHER_BINARY_NAME}.js`);
...
const command = Command.make(
  "build-npm-platform-packages",
  {
    archivesDir: Flag.String("archives-dir").pipe(
      Flag.withDescription(
        "Directory holding the release's t3-alpha-<version>-<platform> archives.",
      ),
    ),
  ...
  Command.withDescription(
    "Build t3code-alpha and t3code-alpha-<platform> npm packages from CLI release archives.",
  ),
);
